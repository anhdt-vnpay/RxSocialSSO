package com.anhdt.rxsocialsso.library.auth;
...
import com.anhdt.rxsocialsso.library.App;
...
public class RxSocialAuth {
    ...
    public PublishSubject<RxStatus> signOut(FragmentActivity activity) {
        ...
        Observable.merge(rxGoogleSignOut, rxFacebookSignOut, rxSmartLockDisableAutoSignin)
                .subscribe(new Consumer<RxStatus>() {
                    @Override
                    public void accept(RxStatus rxStatus) throws Exception {
                        mStatusObserver.onNext(rxStatus);
                        mStatusObserver.onComplete();
                    }
                });
